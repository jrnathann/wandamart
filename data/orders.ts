...
import { OrderTracking, OrderStatus, TrackingCheckpoint, OrderItem } from "@/types/OrderTracking";

// Helper to generate checkpoints
const generateCheckpoints = (deliveryZone: string, status: OrderStatus): TrackingCheckpoint[] => {
    const now = new Date();
    ...
    const checkpoints: TrackingCheckpoint[] = [
        { location: "Shopici Warehouse", time: prepTime, status: "En préparation" },
    ];

    if (status === "En route" || status === "Livré") {
        checkpoints.push({ location: `Hub ${deliveryZone}`, time: departTime, status: "En route" });
    }

    if (status === "Livré") {
        checkpoints.push({ location: deliveryZone, time: deliveredTime, status: "Livré" });
    }
...
export const orders: OrderTracking[] = [
    ...
    },
    {
        ...
        checkpoints: generateCheckpoints("Douala", "En route"),
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
    },
    {
...
export const getOrdersByCustomerPhone = (phone: string): OrderTracking[] =>
    orders.filter(o => o.customer.phone === phone);

// UPDATED: Now accepts array of items
export const addOrder = (
    ...
    const generateOrderId = (): string => {
        let id: string;
        do {
            id = `order_${Math.floor(10000 + Math.random() * 90000)}`; // 10000-99999
        } while (orders.find(o => o.id === id)); // ensure no duplicates
        return id;
    };
